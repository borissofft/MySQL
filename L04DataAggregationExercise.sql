...
WHERE `manager_id` = 42;
UPDATE `average_salaries` SET `salary` = `salary` + 5000
WHERE `department_id` = 1;
SELECT `department_id`, AVG(`salary`) AS 'avg_salary' FROM `average_salaries`
GROUP BY `department_id`
ORDER BY `department_id`;
...
SELECT `emp`.`department_id`, MAX(`emp`.`salary`) AS 'third_highest_salary' FROM `employees` AS `emp`
JOIN (SELECT `e`.`department_id`, MAX(`e`.`salary`) AS `max_salary` FROM `employees` AS `e`
JOIN (SELECT `e`.`department_id`, MAX(`e`.`salary`) AS `max_salary`
FROM `employees` AS `e`
...
FROM `employees` AS `e`
JOIN (SELECT `department_id`, AVG(`salary`) AS 'dep_avg_salary' FROM `employees` GROUP BY `department_id`) AS `avrg`
