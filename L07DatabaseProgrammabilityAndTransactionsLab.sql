...
JOIN `towns` AS `t`
ON `a`.`town_id` = `t`.`town_id` AND `t`.`name` = town_name);
RETURN e_count;
...
AFTER DELETE   -- apply on DELETE ivent
ON `employees` -- Trigger is attached to a table. We do not call triggers explicitly.
FOR EACH ROW
BEGIN
