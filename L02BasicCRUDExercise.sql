...
SELECT `country_name`, `country_code`, `currency_code` AS 'currency' FROM `countries`
WHERE `currency_code` IS NOT NULL
ORDER BY `country_name`;
ALTER TABLE `country_currencies`
CHANGE COLUMN `currency` `currency` VARCHAR(20);
UPDATE `country_currencies`
SET `currency` = IF(`currency` = 'EUR' , 'Euro' , 'Not Euro');
