...
ON `e`.`employee_id` = `ep`.`employee_id`
JOIN `projects` AS `p`
ON `ep`.`project_id` = `p`.`project_id` AND DATE(`start_date`) > '2002-08-13' AND `end_date` IS NULL
ORDER BY `first_name`, `project_name` LIMIT 5;

-- Task 8

SELECT `e`.`employee_id`, `e`.`first_name`, 
IF(YEAR(`start_date`) >= '2005', NULL ,`p`.`name`) AS 'project_name' FROM `employees` AS `e`
JOIN `employees_projects` AS `ep`
...
COUNT(*) AS `cn` FROM `countries` AS `c2`
...
FROM (SELECT `mc`.`country_code` AS 'mc_country_code'
