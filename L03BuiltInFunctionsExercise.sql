...
SELECT * FROM `towns`
WHERE `name` NOT REGEXP '^B'
AND `name` NOT REGEXP '^D'
AND `name` NOT REGEXP '^R'
ORDER BY `name`;
...
-- WHERE CHAR_LENGTH(`country_name`) - CHAR_LENGTH(REPLACE(`country_name`, 'a' OR 'A', '')) >= 3 -- Ne raboti ima greshka
ORDER BY `iso_code`;

-- Task 11

-- SELECT `peak_name`, `river_name`, LOWER(CONCAT(SUBSTRING(`peak_name`, 1, CHAR_LENGTH(`peak_name`) - 1), `river_name`)) AS `mix`
